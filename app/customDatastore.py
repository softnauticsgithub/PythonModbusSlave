"""
Custom Modbus datastore implementation that supports configurable register maps and event-driven updates.
"""
...
class ConfigurableDataBlock(ModbusSequentialDataBlock):
    """
    Custom Modbus datastore that supports configurable register maps and event-driven updates.
    The register map defines the properties of each register, such as access permissions (read-only, write-only, read-write) and data types.
    """
    ...
    def set_values_from_central_engine(self, address, values):
        """
        This method is intended to be called by the central engine (e.g. via MQTT) to update register values based on incoming updates from the central system.
        """
        # TODO: Implement pending logic to handle incoming updates from central system (e.g. via MQTT) and update the datastore accordingly.
        # This may involve validating the incoming address and value, checking access permissions, and then updating the register values without triggering another event publication (to avoid infinite loops).
        super().setValues(address, values)

    def getValues(self, address, count=1):
        """
        Override getValues to enforce read-only access permissions.
        """
